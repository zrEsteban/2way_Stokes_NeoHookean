...
const scalarField& pdmsElasticWallPressureFvPatchScalarField::rhoSolidHs() const
{
    if (rhoSolidHsPtr_.empty())
    {
        ...
        // Updated-Lagrangian solids expose rho_0; other models expose rho.
        const word rhoName
        (
            fsi.solidMesh().foundObject<volScalarField>("rho")
          ? "rho"
          : "rho_0"
        );

        if (!fsi.solidMesh().foundObject<volScalarField>(rhoName))
        {
            FatalErrorInFunction
                << "Neither rho nor rho_0 is registered in the solid region"
                << abort(FatalError);
        }
        ...
        Info<< type() << ' ' << patch().name()
            << ": interface mass/area min=" << min(rhoHs)
            << ", max=" << max(rhoHs)
            << ", mean=" << average(rhoHs)
            << "; effective inertial impedance mean="
            << average(rhoHs)/mesh.time().deltaTValue() << endl;
...
pdmsElasticWallPressureFvPatchScalarField::pdmsElasticWallPressureFvPatchScalarField
(
    const pdmsElasticWallPressureFvPatchScalarField& ptf,
    const fvPatch& p,
    const DimensionedField<scalar, volMesh>& iF,
...
pdmsElasticWallPressureFvPatchScalarField::pdmsElasticWallPressureFvPatchScalarField
(
    const pdmsElasticWallPressureFvPatchScalarField& pivpvf
)
:
...
void pdmsElasticWallPressureFvPatchScalarField::autoMap
(
...
void pdmsElasticWallPressureFvPatchScalarField::rmap
(
...
void pdmsElasticWallPressureFvPatchScalarField::patchFlux
(
...
void pdmsElasticWallPressureFvPatchScalarField::write(Ostream& os) const
{
...
(
    fvPatchScalarField,
    pdmsElasticWallPressureFvPatchScalarField
);
